...
Object eval(Object l, Env env) {
  ...
  if (symbolp(function)) {
    ...
    if (f_str == lisp_let) {
      return do_let(cdr(l), env);
    }
    if (f_str == lisp_lambda) {
      return do_lambda(l);
    }
    if (f_str == lisp_quote) {
      return do_quote(cdr(l));
    }
    if (f_str == lisp_if) {
      return do_if(cdr(l), env);
    }
    if (f_str == lisp_or) {
      return do_or(cdr(l), env);
    }
    if (f_str == lisp_and) {
      return do_and(cdr(l), env);
    }
    if (f_str == lisp_not) {
      return do_not(cdr(l), env);
    }
    if (f_str == lisp_cond) {
      return do_cond(cdr(l), env);
    }
    if (f_str == lisp_display) {
      return do_display(cdr(l));
    }
    if (f_str == lisp_eval) {
      return do_eval(cadr(l), env);
    }
